...
public class ScrapingWorker {
    ...
    private final NumberFormat format = NumberFormat.getInstance(Locale.GERMAN);
    ...
    private Optional<String> extractBsr(Document doc) throws ParseException {
        ...
        val targetSpan = allSpans.stream().filter(element -> element.text().contains("Amazon Bestseller-Rang")).findFirst();

        if (targetSpan.isPresent()) {
            val bsr = targetSpan.get().parent().childNodes().stream().filter(child -> child instanceof TextNode && ((TextNode) child).text().matches(BSR_REGEX)).findFirst();
            if (!bsr.isPresent()) {
                return Optional.empty();
            }
